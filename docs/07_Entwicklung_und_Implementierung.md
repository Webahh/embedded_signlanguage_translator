...
Die beiden Pipelines werden durch `CAM_DisplayPipe_Start()` und `CAM_NNPipe_Start()` gestartet. Für beide Ausgabepfade wird jeweils ein Frame-Interrupt aktiviert. Dieser signalisiert, dass die Übertragung eines vollständigen Bildes in den vorgesehenen Puffer abgeschlossen wurde. Anschließend kann der jeweilige Puffer für die Displayausgabe beziehungsweise die KI-Verarbeitung verwendet werden. Die konkreten Bildverarbeitungsschritte innerhalb der DCMIPP werden in Abschnitt 7.8.1 beschrieben. Die Zuordnung und Verwaltung der verwendeten Bildpuffer ist Gegenstand des folgenden Abschnitts.
...
![[LCD-TFT_Layers.png.png | center | 6000]]
Abbildung X: Zusammenfügen von zwei Layern (STMicroelectronics, 2025)
...
Neben der Begrenzungsbox stellt das Modell mehrere Schlüsselpunkte der Hand bereit. In Anlehnung an die MediaPipe-Referenzimplementierung werden zwei dieser Punkte zur Bestimmung der Handorientierung verwendet. Der zwischen ihnen gebildete Richtungsvektor wird auf einen Zielwinkel von 90° und damit auf die vertikale Achse der Region ausgerichtet (Google (MEDIAPIPIE, o. D. https://github.com/google-ai-edge/mediapipe/blob/master/mediapipe/modules/hand_landmark/palm_detection_detection_to_roi.pbtxt). Da die $y$-Achse in Bildkoordinaten nach unten zeigt, wird ihre Richtungsdifferenz mit umgekehrtem Vorzeichen berücksichtigt. Der Rotationswinkel ergibt sich damit zu:
