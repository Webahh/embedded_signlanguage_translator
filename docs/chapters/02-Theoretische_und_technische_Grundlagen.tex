...
\begin{itemize}
    ...
    \item Taktgeber: Interner Oszillator (HSI, engl.\ \texttt{High-Speed Internal}) oder externer Quarz (HSE, engl.\ \texttt{High-Speed External}) als Zeitbasis für die CPU und Peripherie.
    \item Peripherie-Einheiten: Hardwaremodule für Ein-/Ausgabe, Kommunikation und Zeitsteuerung (siehe Abschnitt \ref{sub:peripherieschnittstellen}).
\end{itemize}
...
Mikrocontroller stellen verschiedene Hardware-Schnittstellen bereit, um mit der Umwelt zu interagieren. Die wesentlichen Peripherietypen sind \cite{STM32N6570DKProductSTMicroelectronics, Mikrocontroller}:
...
Die Speicherarchitektur eingebetteter Systeme weicht wesentlich von klassischen Rechnern ab. Die wesentlichen Speichertypen sind \cite[s. 193-227 (Wolf)]{wolfComputersComponentsPrinciples2012, STM32N6570DKProductSTMicroelectronics, rm0486}:
...
\begin{itemize}
    ...
    \item \texttt{SRAM (On-Chip): } Flüchtig, sehr schneller Zugriff (im Takt der CPU). Dient als Hauptspeicher für Variablen, Heap und Stack. Auf ausgewählten Cortex-Systemen ist SRAM in mehrere Banken aufgeteilt (AXISRAM), um parallele Zugriffe durch CPU, DMA und NPU zu ermöglichen.
...
\begin{itemize}
    ...
    \item \texttt{Register-Level-Programmierung: }Im Bare-Metal-Ansatz werden Peripherie-Einheiten über Memory-Mapped I/O-Register konfiguriert. Jedes Register hat eine fest definierte Adresse im Speicherbereich. Die Konfiguration erfolgt durch Setzen oder Löschen einzelner Bits mit Bitmasken. Beispielsweise wird ein GPIO-Pin als Ausgang konfiguriert, indem im Mode-Register die entsprechenden Bits gesetzt werden. Der Vorteil gegenüber einer Hardware-Abstraktionsschicht (HAL) liegt in der vollen Kontrolle über die Ausführungszeit und den Speicherverbrauch. \cite[ch. ~2]{GbatiIsrael2024BECP}
    \item \texttt{Vorteile und Trade-Offs: }Bare-Metal bietet minimale Latenz, minimalen Speicherverbrauch und deterministisches Verhalten. Die Nachteile liegen in der höheren Entwicklungskomplexität: Es gibt kein Thread-Management, keine Synchronisationsprimitive (Mutexe, Semaphore) und keine automatische Ressourcenverwaltung. Für ressourcenbeschränkte Systeme mit klar definiertem Ablauf überwiegen jedoch die Vorteile \cite{GbatiIsrael2024BECP, Mikrocontroller}.
\end{itemize}
...
\begin{itemize}
    \item \texttt{DMA:} Entlastet die CPU bei Datenübertragungen.
