...
\begin{figure}[H]
    ...
    \caption{Absolute und zeilennormierte Konfusionsmatrix}
    \label{fig:confusion-matrix}
\end{figure}

Die Konfusionsmatrizen basierend auf 371.547 Testdaten zeigt eine 99,28\% Accuracy. Die Matrix zeigt verschiedene Probleme des Modells auf. \texttt{NONE} (Klasse 0) ist mit 72,6\% die mit Abstand schwächste Klasse. Sie wird häufig mit B (\texttt{388x} | 5\%), G, M, R (jeweils ca. \texttt{275x} | 3\%), A (2\%) und D, F, H, N, S, T, W, X, SCH (jeweils ca. \texttt{50} | 1\%) verwechselt. Es werden leere/ungesehene Hände oft als Buchstaben klassifiziert.
 ...
 Das Modell ist bis auf NONE zuverlässig (99-100\% auf 24 von 26 Klassen). Die \texttt{NONE}-\-Erkennung ist mit ca. 73\% der größte Schwachpunkt. Eine weitere und ausführlichere Aufschlüsselung ist im Anhang \ref{tab:confusion-matrix} zu finden.
...
Die Ende-zu-Ende-Latenz beträgt 60–-80 ms. Die Analyse der einzelnen Modellinferenzzeiten zeigt, dass die reine Ausführungszeit der Modelle etwa 44 ms beträgt. Die verbleibenden 16–-36 ms sind demnach der Vor- und Nachverarbeitung der Inferenzdaten sowie weiteren Verarbeitungsschritten innerhalb der Pipeline zuzuordnen.
