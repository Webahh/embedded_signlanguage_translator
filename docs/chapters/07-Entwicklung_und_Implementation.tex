...
Gesture: Repräsentation einer vollständigen Geste
\begin{itemize}
...
Es handelt sich um keine Binärklassifikation, da mehr als zwei Klassen zu unterscheiden sind. Mit den 26 Klassen (NONE, A–Y ohne J und Z, SCH) handelt es sich um eine Multiklassen-Klassifikation.
...
\paragraph{Klassenverteilung:} Der Datensatz umfasst 26 Klassen. \texttt{NONE} (Index 0) für keine erkannte Geste, \texttt{A--Y} (Indizes 1--24) für 24 statische Buchstaben (\texttt{J} und \texttt{Z} ausgeschlossen, da diese dynamische Handbewegungen erfordern) sowie \texttt{SCH} (Index 25) als Sonderzeichen. Die Klassenverteilung ist leicht ungleichmäßig (16 Klassen mit 185, 8 Klassen mit 148, \texttt{NONE} mit 111, \texttt{F} mit 110 Dateien).
...
\begin{itemize}
    \item Intra-Hand-Abhängigkeiten: Korrelation zwischen benachbarten Landmarks des selben Fingers (z.B. die Auslenkung von PIP und DIP des Indexfingers), zwischen verschiedenen Fingern einer Hand sowie die Orientierung der Hand.
...
Das folgende Kapitel beschreibt die Quantisierung des Fingeralphabet-Modell in TensorFlow Format, die Konvertierung der quantisierten Handdetektion-, Handlandmark- und Fingeralphabet- Modelle in NPU ausführbare Binaries. Im letzten Schritt wird das Flashen der Modelle auf den STM32N650DK beschrieben.
...
\texttt{options} \newline
Definiert pro Modell welche Compilerflags verwendet werden sollen. Folgend werden die verwendeten Compiler flags erläutert:
...
\begin{lstlisting}[language=json, caption={Globale Speichereinschränkung in .mpool}, label={lst:mpool-params}]
...
Die vier AXISRAM-Bänke sind On-Chip-SRAM-Speicher mit hohem Durchsatz und geringer Latenz, die für Laufzeit-Aktivierungen (Zwischentensoren) genutzt werden. \texttt{octoFlash} ist zur Laufzeit schreibgeschützt (ACC\_READ) und speichert die Modellgewichte bzw.\ -konstanten.
...
Die Modellpartitionen werden als Binärdateien (\texttt{.bin}) mit dem \texttt{STM32\_Programmer\_CLI} unter Angabe des externen Loaders \texttt{MX66UW1G45G\_STM32N6570-DK.stldr} an ihre Memory-Mapped-Adresse geschrieben und anschließend verifiziert:

\begin{lstlisting}[language=bash, caption={Flashen von Modellen mittels STM32Programmer--CLI}, label={lst:flash-cmd}]
...
Als Lösung wurde ein prioritätsbasierter preemptiver Scheduler auf der ARM Cortex-M55 Architektur implementiert. Der Scheduler verwaltet maximal 9 Tasks, wobei Slot 8 fest dem Idle-Task zugewiesen ist, der im WFI-Zustand (Wait For Interrupt) verharrt und bei Bedarf Statistiken ausgibt.

Zeitgeber (Tick-Source): Als Zeitsystem dient TIM7 mit einer Periode von 1 ms, konfiguriert über APB1 mit 200 MHz Taktfrequenz. Der Timer generiert im \texttt{TIM7\_IRQHandler} einen Tick, der den System-Tick-Zähler (\texttt{\_sys\_tick\_ms}) erhöt und bei abgelaufener Periode die jeweiligen Tasks in den TaskReady-Zustand überführt, gefolgt von einer PendSV-Anforderung zur Kontextwahl.
...
\begin{itemize}
    ...
    \item \texttt{Ready}: Task ist ausführbereit. Wartet auf Scheduler Auswahl.
    \item \texttt{Blocked}: Task wartet auf Ablauf seiner Scheduling-Periode
    \item \texttt{Suspended}: Task wird an der Ausführung gehindert  bis ein externes Event diesen fortsetzt (z.B. DMA2D pausiert Task bis die Speicheroperation vollendet ist).
\end{itemize}
...
Die Kontextwahl ist der Kernmechanismus des Schedulers und erfolgt auf ARM Cortex-M55 zu drei Zeitpunken: Dem anfänglichen Bootstrap des ersten Tasks, der periodischen Tick-gesteuerten Aktivierung via TIM7\_IRQHandler, und der eigentliche Kontextwechsel via \linebreak PendSV\_Handler.

\newpage

\subsubsection{Anfänglicher Context-Start (Bootstrapping)}\label{ssub:bootstrap}
Der Start des ersten Tasks erfolgt nicht über PendSV, sondern über einen SVC-Aufruf (Supervisor Call). \texttt{SCHEDULER\_Tasks\_run()}  bildet diesen Startpunkt und läuft wie folgt ab:
...
SCHEDULER_Tasks_run()
  ...
  |- NVIC-Prioritäten konfigurieren:
  |    SVCall  -> 0x00 (höchste, für Bootstrap)
  |    TIM7    -> 0x80 (Scheduler-Tick)
  |    PendSV  -> 0xFF (niedrigste, Context-Switch)
  |- FPU lazy-stacking aktivieren (ASPEN + LSPEN)
  |- _Task_SelectNext() -> wählt ersten bereiten Task
...
Jede Millisekunde löst der TIM7-Interrupt aus und steuert den Scheduling-Ablauf. Jeder registrierte Task welcher nicht \texttt{TaskDeleted} oder \texttt{TaskSuspended} ist wird auf Ablauf des Ausführungsintervalls geprüft. Ein Task dessen Intervall abgelaufen ist wird in den \texttt{TaskReady} Zustand versetzt. Dies ist notwendig um den Task für den Scheduling-Algorithmus auswählbar zu machen. Im Anschluss wird die PendSV Ausführung angefordert.
...
Die eigentliche Kontextwahl erfolgt im \texttt{PendSV\_Handler} in reiner Assembler-Implementierung. PendSV hat die niedrigste NVIC-Priorität (0xFF), wird somit erst nach Abschluss aller anderen Interrupts ausgeführt.
...
PendSV\_Handler:
    ...
    15. IRQs aktivieren
    16. bx lr                                                  
       -> EXC_RETURN führt zur Task-Rückkehr:
            - Thread-Modus
            - PSP als Stack-Pointer
            - Stack-Frame automatisch popped (R0-R3,R12,LR,PC)
            - CPU springt zu PC (nächste Instruktion des Tasks)
...
\paragraph{Ausgabe}
Alle 1000\,ms, getriggert durch den \texttt{TIM7\_IRQHandler}, wird \texttt{\_stats\_pending} gesetzt und im Idle-Task die Funktion \texttt{\_PrintStats()} aufgerufen. Diese erzeugt eine tabellarische Ausgabe mit Informationen zu den ausgeführten Tasks. Sie enthält den Namen, die Anzahl der insgesamt verwendeten CPU-Zyklen sowie die minimale, maximale und durchschnittliche Anzahl an CPU-Zyklen bis zur Beendigung des Tasks, den Aktiv-Anteil in Prozent an der verwendeten CPU-Zeit, die Anzahl der Unterbrechung durch andere Tasks und die Anzahl der Aufrufe des Tasks im Zeitraum. Zusätzlich werden ISR-Overhead und die CPU-Auslastung in Prozent ausgegeben. Die Statistik wird nach jeder Ausgabe zurückgesetzt.
...
Für die Gewährleistung einer intuitiven Bedienbarkeit sowie zur Bereitstellung von Rückmeldungen über durch den Nutzer ausgeführte Interaktionen sind visuelle Indikatoren auf dem Display erforderlich. Das Projekt umfasst daher sowohl die Visualisierung der Erkennungsergebnisse als auch die Bereitstellung einer grafischen Benutzeroberfläche, über die diese Ergebnisse dargestellt und vom Nutzer kontrolliert beziehungsweise verwaltet werden können.

\subsection{Darstellung der Erkennungsergebnisse}\label{sub:darstellung-ergebnisse}

Im Rahmen der Entwicklung mussten die Inferenzergebnisse in einer intuitiv verständlichen Repräsentation visualisiert werden. Die Benutzeroberfläche ermöglicht es, deren Sichtbarkeit sowie ihren Aktivierungszustand zu steuern.

\subsubsection{Handdedektion}\label{ssub:darstellung-palm}

Das Palm-Detektionsmodell erzeugt eine einzelne Bounding-Box um die erkannte Hand. Dieser wird als rotes rotiertes Rechteck auf dem Display dargestellt. Die vier Eckpunkte der ROI (Region of Interest) werden aus den Normalisierten-Koordinaten im 192x192 Inferenzraum in den 800x480 Displayraum transformiert. Dabei werden die unterschiedlichen Crop-Pipelines der Kamera (Sensorkoordinaten vs. Displaykoordinaten) berücksichtigt. Die ROI wird um einen Faktor von 2{,}6 vergrößert und um $-0{,}5$ in y-Richtung verschoben, um die Gesamtheit der Hand einzuschließen. Die Rotation wird aus den Handgelenkpunkten (0 und 2 | Siehe \ref{fig:hand-landmarks}) berechnet. Die Darstellung erfolgt über vier Linien, die die Eckpunkte des Rechtecks in Rotationsrichtung verbinden. Jede Linie wird mit dem Bresenham-Algorithmus gerendert und die Eckpunkte werden auf die Displaygrenzen begrenzt, um Überläufe zu verhindern. Im reinen Palm-Modus (\texttt{ai\_mode == 0}) wird ausschließlich die ROI dargestellt. Im Tracking-Modus wird die ROI zusätzlich zu den Handlandmarks gezeichnet. Um Flackern zu vermeiden, wird die ROI bei jedem Frame-Interrupt (im \texttt{DCMIPP\_PIPE\_FrameEventCallback}) in den nächsten Display-Buffer kopiert und erneut gezeichnet, falls seit dem letzten gültigen Inferenzergebnis kein neues vorliegt.
...
Die Verbindungslinien folgen einer festen Knochenstruktur, die 23 Verbindungen umfasst: Für jeden Finger wird die Kette vom Handgelenk über die Mittelhand- und Fingerknochen bis zur Fingerspitze dargestellt, ergänzt durch Verbindungen zwischen den Mittelhandgelenken. Jede Linie wird mit dem Bresenham-Algorithmus gerendert. Linien, deren Länge den Schwellwert von 180\,Pixeln überschreiten, werden als unplausibel verworfen und nicht gezeichnet.
...
Das Ergebnis des Fingeralphabetmodell wird über \texttt{USART} und als Textbox auf dem Display ausgegeben. Ist kein gültiges Inferenzergebnis vorliegt wird statt des Ergebnisses \linebreak \texttt{SEARCHING...} angezeigt.
