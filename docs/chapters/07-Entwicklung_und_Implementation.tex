...
Um einen Trainingsprozess durchlaufen zu können müssen Hyperparameter bestimmt werden (vgl.\ Abschnitt~\ref{sub:parameter-hyperparameter}). Zentral sind Optimizer, Lernrate, Verlustfunktion, Epochen, Trainingssplit, Batch size und Early stopping.
...
\begin{lstlisting}[language=bash, caption={Flashen von Modellen mittels STM32\_Programmer\_CLI}, label={lst:flash-cmd}]
...
Die Kontextwahl ist der Kernmechanismus des Schedulers und erfolgt auf ARM Cortex-M55 zu drei Zeitpunken: Dem anfänglichen Bootstrap des ersten Tasks, der periodischen Tick-gesteuerten Aktivierung via TIM7\_IRQHandler, und der eigentliche Kontextwächsel via PendSV\_Handler.
...
Jede Millisekunde löst der TIM7-Interrupt aus und steuert den Scheduling-Ablauf. Jeder registrierte Task welcher nicht \texttt{TaskDeleted} oder \texttt{TaskSuspended} ist wird auf Ablauf des Ausführungs Intervalls geprüft. Ein Task dessen Intervall abgelaufen ist wird in \texttt{TaskReady} Zustand versetzt. Dies ist notwendig um den Task für den scheduling Algorithmus auswählbar zu machen. Im Anschluss wird die PendSV Ausführung angefordert.
...
Das Ergebnis des Fingeralphabetmodell wird über \texttt{USART} und als Textbox auf dem Display ausgegeben.
