...
from src.model_pipeline.models.palm_anchors import (
    PALM_ANCHORS,
)
...
class PalmDetector:
    def __init__(
        ...
        self._interpreter = load_model(
            model_path
        )

        self._input_details = (
            self._interpreter.get_input_details()[0]
        )
    ...
    def _decode_detection(
        ...
        for keypoint_index in range(7):
            ...
            keypoints[keypoint_index, 0] = (
                raw[offset] / MODEL_SIZE
                + anchor_x
            )

            keypoints[keypoint_index, 1] = (
                raw[offset + 1] / MODEL_SIZE
                + anchor_y
            )
