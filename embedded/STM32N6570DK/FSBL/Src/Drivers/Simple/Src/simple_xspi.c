...
#define XSPI_FIFO_THRESHOLD     			7
#define XSPI_CLEAR_ADDRESS      			0
#define XSPI_MM_APMS             			1U
#define XSPI_MM_FMODE            			3

#define APS256XX_MR0_ADDR	        		0x00
#define APS256XX_MR4_ADDR       		 	0x04
#define APS256XX_MR8_ADDR		        	0x08
#define APS256XX_MR0_VALUE       			0x30 /* Drive Strength Full, Read Latency 7, Fixed Latency */
#define APS256XX_MR4_VALUE   		    	0x20 /* PASR, Refresh Frequency, Write Latency Code */
#define APS256XX_MR8_VALUE   		    	0x40 /* Burst Length, Burst Type, X16 */
#define APS256XX_REG_WRITE_DLR			    0x1
#define APS256XX_REG_WRITE_DUMMY_BYTE       0x0
#define APS256XX_REG_WRITE_CMD              0xC0
#define APS256XX_CMD_LINEAR_BURST_READ      0x20
#define APS256XX_CMD_LINEAR_BURST_WRITE     0xA0
#define APS256XX_READ_DUMMY_CYCLES     	  	6
#define APS256XX_WRITE_DUMMY_CYCLES     	6
...
static void XSPI_Init(XSPI_TypeDef *XSPIX , XSPI_cfg_TypeDef cfg){
    ...
    xspi_ptr->CR = XSPI_FIELD(CR, FTHRES, XSPI_FIFO_THRESHOLD);
    xspi_ptr->TCR = 0;
...
static void XSPI_WriteReg(XSPI_TypeDef *xspi, uint8_t reg_addr, uint8_t value){
    ...
    xspi->CR = XSPI_FIELD(CR, FTHRES, XSPI_FIFO_THRESHOLD)
    		 | XSPI_CR_EN;

    timeout = XSPI_TIMEOUT;
    while (xspi->SR & XSPI_SR_BUSY){
        if (--timeout == 0) break;
    }
    ...
    xspi->IR  = APS256XX_REG_WRITE_CMD;
    xspi->AR  = reg_addr;
    xspi->DLR = APS256XX_REG_WRITE_DLR;

    *((__IO uint8_t*)&xspi->DR) = value;
    *((__IO uint8_t*)&xspi->DR) = APS256XX_REG_WRITE_DUMMY_BYTE;
    ...
    xspi->AR  = XSPI_CLEAR_ADDRESS;
    xspi->FCR = XSPI_FCR_CTCF;
}

/* Write PSRAM (APS256XX) configuration registers */
static void PSRAM_WriteConfig(void){
    XSPI_WriteReg(XSPI1, APS256XX_MR0_ADDR, APS256XX_MR0_VALUE);
    XSPI_WriteReg(XSPI1, APS256XX_MR4_ADDR, APS256XX_MR4_VALUE);
    XSPI_WriteReg(XSPI1, APS256XX_MR8_ADDR, APS256XX_MR8_VALUE);
}

/* Write NOR Flash (MX66UW1G45G) configuration registers */
static void NOR_WriteConfig(void){
    XSPI_WriteReg(XSPI2, APS256XX_MR0_ADDR, APS256XX_MR0_VALUE);
    XSPI_WriteReg(XSPI2, APS256XX_MR4_ADDR, APS256XX_MR4_VALUE);
    XSPI_WriteReg(XSPI2, APS256XX_MR8_ADDR, APS256XX_MR8_VALUE);
}
...
static void XSPI1_EnableMemoryMappedMode(void){
    ...
    XSPI1->WIR  = APS256XX_CMD_LINEAR_BURST_WRITE;
    XSPI1->WTCR = XSPI_FIELD(WTCR, DCYC, APS256XX_WRITE_DUMMY_CYCLES);

    XSPI1->CCR  = XSPI_BuildWCCR_CCR(XSPI_memorymapped_cfg);

    XSPI1->IR   = APS256XX_CMD_LINEAR_BURST_READ;
    XSPI1->TCR  = XSPI_FIELD(TCR, DCYC, APS256XX_READ_DUMMY_CYCLES);

    /* Enable XSPI1 in memory-mapped mode (FMODE=3), FIFO threshold=7 */
    XSPI1->CR = XSPI_FIELD(CR, FMODE,  XSPI_MM_FMODE)
              | XSPI_FIELD(CR, FTHRES, XSPI_FIFO_THRESHOLD)
              | XSPI_CR_EN;
}
...
static void XSPI2_EnableMemoryMappedMode(void){
    ...
    XSPI2->TCR  = XSPI_TCR_DHQC
                | XSPI_FIELD(TCR, DCYC, APS256XX_READ_DUMMY_CYCLES);
    ...
    XSPI2->CR = XSPI_FIELD(CR, FMODE, XSPI_MM_FMODE)
    		  | XSPI_FIELD(CR, APMS, XSPI_MM_APMS)
              | XSPI_CR_EN;
}

/*
 * PSRAM_Init – full initialisation sequence for PSRAM on XSPI1
 *  1) Enable/reset XSPI1, XSPIM clocks
 *  2) Configure XSPI1 GPIOs, device registers, PSRAM-specific regs
 *  3) Bypasses prescaler = 3 to 0 for 200MHz
 */
 ...
 *
 * FOR NOW not needed and not supported!
 */
